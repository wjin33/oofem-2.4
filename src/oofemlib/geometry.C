...
void PolygonLine :: giveSubPolygon(std :: vector< FloatArray > &oPoints, const double &iXiStart, const double &iXiEnd) const
{
    ...
    if (Points.size()>1){
        for (size_t i = 0; i < Points.size()-1; i++){
            double r= Points[i].distance(Points[i+1]);
            if (r > 1.0e-2){
                oPoints.push_back(Points[i]);
            }
            if (i == Points.size()-2){
                oPoints.push_back(Points[i+1]);
            }
        }
    }else{
        oPoints.push_back(Points[0]);
    }
}
...
void PolygonLine :: givePoints(FloatArray &points)
{
    int nVert = mVertices.size();
    points.resize(nVert * 2);
...
void PolygonLine :: giveAdvancingSection(FloatArray &oCenter, double &length)
{
    int nVert = giveNrVertices();
    oCenter = {
        0.0, 0.0
    ...
    if ( nVert > 2 ) {
        length=giveVertex(nVert).distance(giveVertex(nVert-1));
        oCenter.add(giveVertex(nVert));
        oCenter.add(giveVertex(nVert-1));
        oCenter.times( 1.0 / 2 );
    }
