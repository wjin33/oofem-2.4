...
Subdivision::RS_Triangle::giveEdgeIndex(int iNode, int jNode) {
	...
	if(in==0 || jn==0){
		OOFEM_ERROR("Subdivision::RS_Triangle::giveEdgeIndex - there is no edge connecting %d and %d on element %d",
...
Subdivision::RS_Tetra::giveEdgeIndex(int iNode, int jNode) {
	...
	if(in==0 || jn==0){
		OOFEM_ERROR("Subdivision::RS_Tetra::giveEdgeIndex - there is no edge connecting %d and %d on element %d",
								 iNode, jNode, this->giveNumber());
		return 0;
	}
  ...
  if (irregular_nodes.containsOnlyZeroes()) {
    ...
    */
    ...
    if (irregular_nodes.at(iedge) && irregular_nodes.at(jedge) && irregular_nodes.at(kedge)) {
      ...
      OOFEM_ERROR("Subdivision::RS_Triangle::generate - element %d internal data inconsistency", this->number);
    }
  ...
  for (iside=1; iside<=3; iside++) {
    if (this->neghbours_base_elements.at(iside) < 0) {
      ...
      // test if parentNeighbor has been subdivided
        ...
        if (!found) {
          OOFEM_ERROR ("Subdivision::RS_Triangle::update_neighbours failed for element %d", this->number);
        }
      } else {
    ...
    if (this->neghbours_base_elements.at(iside) < 0) {
      ...
      // test if parentNeighbor has been subdivided
			if (mesh->giveElement(parentNeighbor)->hasIrregulars()) {      // HUHU replace by !isTerminal
				...
				if (!found) {
					OOFEM_ERROR ("Subdivision::RS_Tetra::update_neighbours failed for element %d (side %d, elem %d)", 
  ...
  int problem_size=this->giveNumberOfProcesses();
    ...
    for (in=nnodes_old; in<=nnodes; in++) {
      ...
      if (gnum < 0) {
        ...
        OOFEM_ERROR ("[%d] Subdivision::bisectMesh: zero globnum identified on node %d", myrank, in);
      }
    }
  ...
  while ( _type != SUBDIVISION_END_DATA ) {
    if (_type == SUBDIVISION_SHARED_IRREGULAR_REC_TAG) {
      ...
      OOFEM_LOG_INFO("[%d] Subdivision::unpackSharedIrregulars: received shared node record [%d %d] from %d ...\n",
			...
			elems=iElems->findCommonValuesSorted(*jElems, common, 0);
			if(!elems){
                          OOFEM_WARNING("[%d] Subdivision::unpackSharedIrregulars - no element found sharing nodes %d and %d",myrank,iNode, jNode);
                          pcbuff->read(_type);
                          continue;
			}
...
#ifdef DEBUG_CHECK
			if(!elem->giveSharedEdges()->giveSize()){
				OOFEM_ERROR("Subdivision::unpackSharedIrregulars - element %d sharing nodes %d %d has no shared edges",
										 elem->giveNumber(), iNode, jNode);
			}
			if(!elem->giveSharedEdge(eIndex)){
				OOFEM_ERROR("Subdivision::unpackSharedIrregulars - element %d sharing nodes %d and %d has no shared edge %d",
...
Subdivision::unpackIrregularSharedGlobnums (Subdivision *s, ProcessCommunicator &pc)
{
  ...
  while ( _type != SUBDIVISION_END_DATA ) {
    if (_type == SUBDIVISION_SHARED_IRREGULAR_REC_TAG) {         // KOKO asi zbytecne
			...
			elems=iElems->findCommonValuesSorted(*jElems, common, 0);
			if(!elems){
				OOFEM_ERROR("Subdivision::unpackIrregularSharedGlobnums - no element found sharing nodes %d and %d",
										 iNode, jNode);
			}
...
#ifdef DEBUG_CHECK
			if(!elem->giveSharedEdges()->giveSize()){
				OOFEM_ERROR("Subdivision::unpackIrregularSharedGlobnums - element %d sharing nodes %d %d has no shared edges",
										 elem->giveNumber(), iNode, jNode);
			}
			if(!elem->giveSharedEdge(eIndex)){
				OOFEM_ERROR("Subdivision::unpackIrregularSharedGlobnums - element %d sharing nodes %d and %d has no shared edge %d",
										 elem->giveNumber(), iNode, jNode, eIndex);
			}
			if(!iNum){
				OOFEM_ERROR("Subdivision::unpackIrregularSharedGlobnums - element %d sharing nodes %d %d does not have irregular",
										 elem->giveNumber(), iNode, jNode);
			}
#endif
