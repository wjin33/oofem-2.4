...
NonlocalMaterialExtensionInterface :: computeWeightFunction(const FloatArray &src, const FloatArray &coord)
{
    ...
    if (this->cl1 == 0){
        ...
        double x0= src.at(1)*cos(this->angle) + src.at(2)*sin(this->angle);
        double y0=-src.at(1)*sin(this->angle) + src.at(2)*cos(this->angle);

        double x1= coord.at(1)*cos(this->angle) + coord.at(2)*sin(this->angle);
        double y1=-coord.at(1)*sin(this->angle) + coord.at(2)*cos(this->angle);

        // Elliptical Bell shaped function (quartic spline)
        double weight = 1-(pow((x0-x1)/this->cl1, 2)+ pow((y0-y1)/this->cl, 2));
        if (weight <= 0){
            return 0;
        } else{
            return pow(weight,2)/(this->cl*this->cl1 *pi/3.);;
        }
    }
}
...
NonlocalMaterialExtensionInterface :: initializeFrom(InputRecord *ir)
{
    ...
    if ( ir->hasField(_IFT_NonlocalMaterialExtensionInterface_admnl2) ) {
        IR_GIVE_FIELD(ir, cl1, _IFT_NonlocalMaterialExtensionInterface_cl1);
        IR_GIVE_FIELD(ir, angle, _IFT_NonlocalMaterialExtensionInterface_theta);
    }
