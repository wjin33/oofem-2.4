...
Domain *Domain :: Clone()
{
    ...
    inputRec->setField(this->giveNumberOfNonlocalBarriers(),    _IFT_Domain_nbarrier);
    ...
    //Update Barrier vertex in the frist place
    //Then input record in Barrior
    //int nEI = xfemManager->giveNumberOfEnrichmentItems();
    int i=1;
    for ( auto &barrier: nonlocalBarrierList ) {
            EnrichmentItem *ei = xfemManager->giveEnrichmentItem(i);
            FloatArray PointsCoords;
            ei->updateNonlocalBarrier(PointsCoords);
            barrier->UpdateVertices(PointsCoords);
            dataReader.insertInputRecord(DataReader :: IR_matRec, new DynamicInputRecord( *barrier ));
            i++;
    }
