...
VTKXMLExportModule :: doOutput(TimeStep *tStep, bool forcedOutput)
{
    ...
    if ( !this->particleExportFlag ) {
        ...
        // Prepare for cutted xfem element output
        ...
        this->initRegionNodeNumbering(mapG2L, mapL2G, numNodes, numRegionEl, d, tStep, 1);

        std :: vector< std :: vector< FloatArray > >smoothedIS; // [field][node][valArray] ;

        int field = internalVarsToExport.giveSize();
        smoothedIS.resize(field);
        for ( int i = 1; i <= field; i++ ) {
            smoothedIS[ i - 1 ].resize(numNodes);
        }
        for ( int i = 1; i <= field; i++ ) {
            for (int inode=1; inode<= numNodes; inode++) {
                FloatArray valueArray = this->defaultVTKPiece.giveInternalVarInNode(i, inode);
                smoothedIS [ i - 1 ] [ inode - 1 ] = std :: move(valueArray);
            }
        }
        ...
        for ( int pieceNum = 1; pieceNum <= nPiecesToExport; pieceNum++ ) {
            ...
            for ( int i = 1; i <= elements.giveSize(); i++ ) {
                ...
                // support for output internal state variables for elements cutted by fractures
                ...
                int numCells=0;
                if ( interface ) {
                    interface->giveNumberofmSubTri(numCells);
                    if (numCells != 0){

                        IntArray cellNodes;
                        // Set the connectivity
                        this->giveElementCell(cellNodes, el);  // node numbering from global numbering
                        // Map from global to local node numbers for the current vtkpiece
                        ...
                        std :: vector< std :: vector< FloatArray > >ElementNodeIS; // [field][node][valArray] ;

                        ElementNodeIS.resize(field);
                        for ( int i = 1; i <= field; i++ ) {
                            ElementNodeIS[ i - 1 ].resize(numElNodes);
                        }


                        for ( int i = 1; i <= field; i++ ) {
                            for (int j=1; j<= numElNodes; j++) {
                                FloatArray valueArray = smoothedIS  [ i - 1 ] [ connectivity.at(j) - 1 ];
                                ElementNodeIS [ i - 1 ] [ j - 1 ] = std :: move(valueArray);
                            }
                        }
                        // Clear object so it can be filled with new data for the next piece
                        //vtkPiece.clear();

                        interface->giveXFEMExportData(this->defaultVTKPieces,ElementNodeIS, this->primaryVarsToExport, this->internalVarsToExport, this->cellVarsToExport, tStep);
                        for ( int j = 0; j < ( int ) this->defaultVTKPieces.size(); j++ ) {
                            this->writeVTKPiece(this->defaultVTKPieces [ j ], tStep);
                        }
                    }
   ...
   if ( numNodes > 0 && numRegionEl > 0 ) {
        ...
        for ( int ei = 1; ei <= elems.giveSize(); ei++ ) {
            ...
            if ( elem->giveParallelMode() != Element_local ) {
             ...
             // Only output the element do not cutted by fracture
            int numCells=0;
            VTKXMLExportModuleElementInterface *interface =
                                              ...
                                              Domain *domain, TimeStep *tStep, int reg)
{
    ...
    for ( int ie = 1; ie <= elements.giveSize(); ie++ ) {
        ...
        int numCells=0;
        VTKXMLExportModuleElementInterface *interface =
            static_cast< VTKXMLExportModuleElementInterface * >( element->giveInterface(VTKXMLExportModuleElementInterfaceType) );
        if ( interface ) {
            interface->giveNumberofmSubTri(numCells);
            if(numCells == 0) {
                regionSingleCells++;
