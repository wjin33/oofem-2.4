...
#include "../sm/Materials/InterfaceMaterials/intmatpprinadm1.h"
...
Element :: postInitialize()
{
    ...
    if ( iRule ) {
        for ( GaussPoint *gp: *iRule ) {
            gp->setgausspointvolume(this->computeVolumeAround(gp));
        }
    }
}
...
void
Element :: checkmaterial(bool &answer)
{
...
int Element :: returGaussPointSize()
{

   XfemStructuralElementInterface *xFemEl = dynamic_cast< XfemStructuralElementInterface * >(this);
   return xFemEl->mpCZIntegrationRules.size();
   /*
   int result = 1;
...
int
Element :: isElementBeingCut(Domain &iOldDom, Domain &iNewDom, Element &elOld, TimeStep &tStep)
{
    int result = 1;

    XfemManager *xMan = iNewDom.giveXfemManager();

    int NumDof=giveNumberOfDofManagers();
    IntArray DofArray=elOld.giveDofManArray();
    int NumEnDof=0;
    ...
    if ( NumEnDof==NumDof){
        ...
        if (elOld.returGaussPointSize() == this->returGaussPointSize()){
            return result;
        }

        result=0;
        // Add gauss points in the old system, may not need right now.
        //elOld.addGaussPoints();
        IntArray nodeElements,MappingVariable;
        ConnectivityTable *ct = iOldDom.giveConnectivityTable();
        ct->giveNodeNeighbourList(nodeElements, DofArray);

        Set Elementset(0, &iOldDom);

        Elementset.setElementList(nodeElements);

        //IntArray DofArray=giveDofManArray();
        NodalRecoveryModel * smoother=classFactory.createNodalRecoveryModel(( NodalRecoveryModel :: NodalRecoveryModelType )2, &iOldDom);
        GaussPoint *gp = integrationRulesArray[0]->getIntegrationPoint(1);
        AnisotropicDamageMaterial1Status *ms = dynamic_cast< AnisotropicDamageMaterial1Status * >(gp->giveMaterialStatus());
        ms->giveInternalStatetype(MappingVariable);

        for (int istat=1; istat <= MappingVariable.giveSize(); istat++){
            InternalStateType Type = ( InternalStateType ) MappingVariable.at(istat);
            smoother->recoverValuesforMapping(Elementset, Type, DofArray, &tStep);
            if (Type == IST_StressTensor){
                XfemStructuralElementInterface *xFemEl = dynamic_cast< XfemStructuralElementInterface * >(this);

                for ( size_t czIndex = 0; czIndex < xFemEl->mpCZIntegrationRules.size(); czIndex++ ) {

                    const FloatArray &x1 = xFemEl->mpCZIntegrationRules [ czIndex ]->getIntegrationPoint(0)->giveGlobalCoordinates();
                    const FloatArray &x2 = xFemEl->mpCZIntegrationRules [ czIndex ]->getIntegrationPoint(1)->giveGlobalCoordinates();
                    FloatArray tangent,normal;
                    normal.resize(2);
                    tangent.beDifferenceOf(x1, x2);
                    if (fabs(tangent.at(1))<1e-5){
                        normal.at(1)=1;
                        normal.at(2)=0;
                    }else if(fabs(tangent.at(2))<1e-5){
                        normal.at(1)=0;
                        normal.at(2)=1;
                    }else{
                        double tem= -tangent.at(1)/tangent.at(2);
                        normal.at(1)=1/pow(1+pow(tem,2),0.5);
                        normal.at(2)=tem/pow(1+pow(tem,2),0.5);
                    }
                    ...
                    for ( GaussPoint *gp: *xFemEl->mpCZIntegrationRules [ czIndex ] ) {
                        ...
                        this->computeLocalCoordinates(locCoordNode, x);
                        ...
                        FloatArray ISTemp=0;
                        for (int elNodeInd = 1; elNodeInd <= NumDof; elNodeInd++){
                            FloatArray Temp=0;
                            const FloatArray *Nodelvalue=NULL;
                            smoother->giveNodalVector(Nodelvalue, elNodeInd);
                            FloatArray Val=0;
                            Val.resize(Nodelvalue->giveSize());
                            for (int i=1; i<=Nodelvalue->giveSize();i++){
                                    Val.at(i)=Nodelvalue->at(i);
                            }
                            Temp.beScaled(N.at(elNodeInd), Val);
                            ISTemp += Temp ;
                        }

                        FloatMatrix stress(2, 2);
                        stress.at(1, 1) = ISTemp.at(1);
                        stress.at(1, 2) = ISTemp.at(6);
                        stress.at(2, 1) = ISTemp.at(6);
                        stress.at(2, 2) = ISTemp.at(2);

                        FloatArray RotStress;
                        RotStress.beProductOf(stress, normal); //Note the difference;

                        IntMatPPRINADM1Status *ppr1Status = dynamic_cast< IntMatPPRINADM1Status * >(gp->giveMaterialStatus());

                        ppr1Status->letTranctionBe( fabs(RotStress.at(1)) );
            ...
            for ( auto &iRule: integrationRulesArray ) {
                for ( GaussPoint *gp: *iRule ) {
                    ...
                    AnisotropicDamageMaterial1Status *adm1Status = dynamic_cast< AnisotropicDamageMaterial1Status * >(gp->giveMaterialStatus());
                    if ( adm1Status == NULL ) {
                    ...
                    if (Type == IST_StrainTensor ){
                        FloatArray value=0;
                        value.resize(4);
                        value.at(1)=ISTemp.at(1);
                        value.at(2)=ISTemp.at(2);
                        value.at(3)=ISTemp.at(3);
                        value.at(4)=ISTemp.at(6);
                        adm1Status->letStrainVectorBe(value);
                        adm1Status->letTempStrainVectorBe(value);
                        ...
                        value.at(3)=ISTemp.at(3);
                        value.at(4)=ISTemp.at(6);
                        adm1Status->letTempStressVectorBe(value);
                        adm1Status->letStressVectorBe(value);
                    }else if (Type == IST_DamageTensor){
                        FloatMatrix value;
                        value.resize(2,2);
                        value.at(1,1)=ISTemp.at(1);
                        value.at(2,2)=ISTemp.at(2);
                        value.at(1,2)=ISTemp.at(6);
                        value.at(2,1)=ISTemp.at(6);
                        adm1Status->setTempDamage(value);
                        adm1Status->setDamage(value);
                    }else if (Type == IST_StressWorkDensity){
                        double value=ISTemp.at(1);
                        adm1Status->setStressWork(value) ;
                        adm1Status->setTempStressWork(value) ;
                    }else if (Type == IST_DissWorkDensity){
                        double value=ISTemp.at(1);
                        adm1Status->setDissWork(value);
                        adm1Status->setTempDissWork(value);
                    }else if (Type == IST_EquivalentStrainC){
                        double value=ISTemp.at(1);
                        adm1Status->setEquivalenstrain_c(value) ;
                        adm1Status->setTempEquivalenstrain_c(value);
                    }else if (Type == IST_EquivalentStrainT){
                        double value=ISTemp.at(1);
                        adm1Status->setEquivalenstrain_t(value);
                        adm1Status->setTempEquivalenstrain_t(value);

                    }else{
                        OOFEM_ERROR("Note supported for the material");
                    }
...
double
Element :: ComputeDissipatedEngergy2()
{
    double ConDisG=0;
        ...
        for ( auto &iRule: integrationRulesArray ) {
            for ( GaussPoint *gp: *iRule ) {
                ...
                double G = adm1Status->giveDissWork();
                double V = this->computeVolumeAround(gp);
...
void
Element :: AssignEngergy2CZ(const double &ConDisG ){

    XfemStructuralElementInterface *xFemEl = dynamic_cast< XfemStructuralElementInterface * >(this);
    for ( size_t czIndex = 0; czIndex < xFemEl->mpCZIntegrationRules.size(); czIndex++ ) {
        const FloatArray &x1 = xFemEl->mpCZIntegrationRules [ czIndex ]->getIntegrationPoint(0)->giveGlobalCoordinates();
        const FloatArray &x2 = xFemEl->mpCZIntegrationRules [ czIndex ]->getIntegrationPoint(1)->giveGlobalCoordinates();
        double glodis = x1.distance(x2);

        const FloatArray &x3 = xFemEl->mpCZIntegrationRules [ czIndex ]->getIntegrationPoint(0)->giveNaturalCoordinates();
        const FloatArray &x4 = xFemEl->mpCZIntegrationRules [ czIndex ]->getIntegrationPoint(1)->giveNaturalCoordinates();
        double locdis = x3.distance(x4);

        double cracksegtionlength= 2*glodis/locdis;

        for ( GaussPoint *gp: *xFemEl->mpCZIntegrationRules [ czIndex ] ) {
            IntMatPPRINADM1Status *ppr1Status = dynamic_cast< IntMatPPRINADM1Status * >(gp->giveMaterialStatus());
            double thickness = this->giveCrossSection()->give(CS_Thickness, gp);
            double TotalAmountG=ppr1Status->TotalG;
            double CoheRemainG=(TotalAmountG*cracksegtionlength*thickness-ConDisG)/(cracksegtionlength*thickness);
            ppr1Status->initializeFrom(CoheRemainG);
        }
    }
