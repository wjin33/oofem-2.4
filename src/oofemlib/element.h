...
public:
    ...
    // especially prepared for mapping state variables of gauss point with elements being cut by crack
    // The following three functions are all for continuum damage to fracture transition
    virtual int isElementBeingCut(Domain &iOldDom, Domain &iNewDom, Element &elOld, TimeStep &tStep);
    virtual void AssignEngergy2CZ(const double &ConDisG);
    double ComputeDissipatedEngergy2();
    void checkmaterial(bool &answer);
    int returGaussPointSize();
