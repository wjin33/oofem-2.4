...
void
SPRNodalRecoveryModel :: recoverValuesforMapping(Set elementSet, InternalStateType type, IntArray &DofArray, TimeStep *tStep)
{
    FloatMatrix a;
    IntArray patchElems;
    int regionValSize;
    ...
    this->clear();
    regionValSize=0;
    int regionDofMans=DofArray.giveSize();

    SPRPatchType regType = this->determinePatchType(elementSet);
    for ( int ipap = 1; ipap <= regionDofMans; ipap++ ) {

        const IntArray *DofManConnectivity = domain->giveConnectivityTable()->giveDofManConnectivityArray(DofArray.at(ipap));
        int nelement=DofManConnectivity->giveSize();
        patchElems.resize(nelement);
        for (int iele=1; iele <= nelement; iele++){
            patchElems.at(iele) =  DofManConnectivity->at(iele);
        }
        this->computePatch(a, patchElems, regionValSize, regType, type, tStep);

        FloatArray P, vals;
        FloatArray *coords = domain->giveNode( DofArray.at(ipap))->giveCoordinates();
        ...
        // assemble values
        FloatArray &nodalVal = this->nodalValList [ ipap ];
        nodalVal.resize(regionValSize);
        for ( int i = 1; i <= regionValSize; i++ ) {
            nodalVal.at(i) = vals.at(i);
        }
    }
