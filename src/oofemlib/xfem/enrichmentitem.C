...
double EnrichmentItem :: CalDissipatedEnergy(Element *El, double &AdvLength, double &interLength)
{
    double ConDisG=0;

    for ( GaussPoint *gp_i: *El->giveDefaultIntegrationRulePtr() ) {
    ////////////////////////////////////////
    // Compute global gp coordinates
        FloatArray N;
        FEInterpolation *interp = El->giveInterpolation();
        interp->evalN( N, gp_i->giveNaturalCoordinates(), FEIElementGeometryWrapper(El) );
        ...
        double normalSignDist;
        evalLevelSetNormal( normalSignDist, globalCoord, N, El->giveDofManArray() );

        double tangSignDist;
        evalLevelSetTangential( tangSignDist, globalCoord, N, El->giveDofManArray() );

        if ( normalSignDist !=0 || tangSignDist!=0 ){
            if ( fabs(normalSignDist) < interLength && tangSignDist <= AdvLength && tangSignDist >=0 ) {

                AnisotropicDamageMaterial1Status *adm1Status = dynamic_cast< AnisotropicDamageMaterial1Status * >(gp_i->giveMaterialStatus());
                if ( adm1Status == NULL ) {
                    OOFEM_ERROR("failed to fetch MaterialStatus.");
                }

                double G = adm1Status->giveDissWork();
                double V = El->computeVolumeAround(gp_i);
