...
void GeometryBasedEI :: updateNonlocalBarrier(FloatArray &points)
{
  mpBasicGeometry->givePoints(points);
}
...
void GeometryBasedEI :: updateNodeEnrMarker(XfemManager &ixFemMan)
{
    ...
    for ( int elNum: elList ) {
        ...
        if ( minPhi * maxPhi < mLevelSetTol ) { // If the level set function changes sign within the element.
            ...
            for ( int edgeIndex = 1; edgeIndex <= numEdges; edgeIndex++ ) {
                ...
                if ( evalLevelSetNormalInNode(levelSetNormalNodeI, niGlob, nodePosI) && evalLevelSetNormalInNode(levelSetNormalNodeJ, njGlob, nodePosJ) ) {
                    if ( levelSetNormalNodeI * levelSetNormalNodeJ < mLevelSetTol ) {
                        ...
                        if ( fabs(tangDist) < 1e-8 ) {
                            tipEdgeintersec++;
                        }else if( tangDist > 1e-8  ){
                            numEdgeIntersec++;
                        }
                    }
                }
            ...
            if ( numEdgeIntersec == 1 && tipEdgeintersec==0 ){
                ...
                FloatArray TipPos1(2),TipPos2(2);
                TipPos1.zero(); TipPos2.zero();\
                bool IsEnd;
                if( elCenter.distance(tipInfoStart.mGlobalCoord) > elCenter.distance(tipInfoEnd.mGlobalCoord)){
                    TipPos1 = tipInfoEnd.mGlobalCoord;
                    TipPos2.add(l,tipInfoEnd.mTangDir);
                    TipPos2 +=TipPos1;
                    IsEnd=true;
                }else {
                    TipPos1 = tipInfoStart.mGlobalCoord;
                    TipPos2.add(l,tipInfoStart.mTangDir);
                    TipPos2 +=TipPos1;
                    IsEnd=false;
                }
                ...
                for ( int edgeIndex = 1; edgeIndex <= numEdges; edgeIndex++ ) {
                    IntArray bNodes;
                    el->giveInterpolation()->boundaryGiveNodes(bNodes, edgeIndex);
                    const FloatArray &nodePosI = el->giveNode(bNodes.at(1))->giveNodeCoordinates();
                    const FloatArray &nodePosJ = el->giveNode(bNodes.at(2))->giveNodeCoordinates();
                    FloatArray IntersecPoint;
                    if (IsLSIntersect(IntersecPoint, TipPos1, TipPos2, nodePosI, nodePosJ )){
                        if (IsEnd){
                            mpBasicGeometry->replaceVertex(IntersecPoint, mpBasicGeometry->giveNrVertices());
                        }else{
                            mpBasicGeometry->replaceVertex(IntersecPoint, 1);
                        }
                        break;
                    }
            ...
            }else if(numEdgeIntersec == 2 || (numEdgeIntersec == 1 && tipEdgeintersec==1)){
...
bool GeometryBasedEI :: IsLSIntersect(FloatArray &answer,const FloatArray &P1,const FloatArray &P2,const FloatArray &Q1,const FloatArray &Q2 )
{
    double t1=max(P1.at(1), P2.at(1));
    double t2=min(Q1.at(1), Q2.at(1));
    double t3=max(Q1.at(1), Q2.at(1));
    double t4=min(P1.at(1), P2.at(1));
    double t5=max(P1.at(2), P2.at(2));
    double t6=min(Q1.at(2), Q2.at(2));
    double t7=max(Q1.at(2), Q2.at(2));
    double t8=min(P1.at(2), P2.at(2));
    if (fabs(t1-t4)<1e-6 && fabs(t2-t3)<1e-6 ){
        return false;
    } else if (fabs(t6-t7)<1e-6 && fabs(t5-t8)<1e-6 ){
        return false;
    }
    double mt1 = (Q1.at(1)-P1.at(1))*(P2.at(2)-P1.at(2))-(Q1.at(2)-P1.at(2))*(P2.at(1)-P1.at(1));
    double mt2 = (P2.at(1)-P1.at(1))*(Q2.at(2)-P1.at(2))-(Q2.at(1)-P1.at(1))*(P2.at(2)-P1.at(2));
    double mt3 = (P1.at(1)-Q1.at(1))*(Q2.at(2)-Q1.at(2))-(Q2.at(1)-Q1.at(1))*(P1.at(2)-Q1.at(2));
    double mt4 = (Q2.at(1)-Q1.at(1))*(P2.at(2)-Q1.at(2))-(Q2.at(2)-Q1.at(2))*(P2.at(1)-Q1.at(1));
    if (fabs(t1-t2)<1e-6){t1=t2;}
    if (fabs(t3-t4)<1e-6){t3=t4;}
    if (fabs(t5-t6)<1e-6){t5=t6;}
    if (fabs(t7-t8)<1e-6){t7=t8;}

    if (fabs(mt1*mt2)<1e-6) {mt1 =0;}
    if (fabs(mt3*mt4)<1e-6) {mt3 =0;}
    if (t1>=t2 && t3>=t4 && t5>=t6 && t7>=t8 && mt1*mt2>=0 && mt3*mt4>=0 ){
        double labmda = ((Q1.at(1)-P1.at(1))*(Q1.at(2)-Q2.at(2))-(Q1.at(2)-P1.at(2))*(Q1.at(1)-Q2.at(1)))/
                        ((P2.at(1)-P1.at(1))*(Q1.at(2)-Q2.at(2))-(P2.at(2)-P1.at(2))*(Q1.at(1)-Q2.at(1)));
        answer.resize(2);
        answer=P1+labmda*(P2-P1);
        return true;
    }else{
        return false;
    }
...
void GeometryBasedEI :: giveAdvancingSection(FloatArray &oCenter, double &length)
{
    // Compute bounding sphere from enrichment domain ...
    mpBasicGeometry->giveAdvancingSection(oCenter, length);
