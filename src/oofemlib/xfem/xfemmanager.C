...
void XfemManager :: giveEnrichItemForElement(FloatMatrix &Center, FloatArray &Lengths)
{
    int Number=giveNumberOfEnrichmentItems();
    Center.resize(Number,2);
    Lengths.resize(Number);
    int ind=1;
    for ( auto &ei: enrichmentItemList ) {
        FloatArray oCenter;
        double length=0;
        ei->giveAdvancingSection(oCenter, length);
        if ( length>0 ){
            Center.at(ind,1)=oCenter.at(1);
            Center.at(ind,2)=oCenter.at(2);
            Lengths.at(ind)=length;
            ind++;
        }
    }
...
double XfemManager :: CalDissipatedEnergy(double &interLength, int &item)
{

    double  DisEnergy;

    SpatialLocalizer *localizer = this->domain->giveSpatialLocalizer();
    EnrichmentItem *ENrichI=this->giveEnrichmentItem(item);
    FloatArray oCenter;
    double AdvLength;

    ENrichI->giveAdvancingSection(oCenter, AdvLength);

    double searchRadius = sqrt(pow(interLength,2)+pow(AdvLength/2,2));
    std :: set< int >elIndices;
    localizer-> giveAllElementsWithIpWithinBox(elIndices, oCenter, searchRadius);

    for ( int elIndex: elIndices ) {
         Element *gpEl = this->domain->giveElement(elIndex);
         DisEnergy +=ENrichI->CalDissipatedEnergy(gpEl, AdvLength, interLength);
    }
    return DisEnergy;
