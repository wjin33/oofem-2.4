...
PolylineNonlocalBarrier :: PolylineNonlocalBarrier(int n, Domain *aDomain) :
    NonlocalBarrier(n, aDomain), mVertices()
...
PolylineNonlocalBarrier :: isActivated(const FloatArray &c1, const FloatArray &c2)
{
    ...
    for (int indx = 1; indx < size; indx++ ) {
        ...
        */
        ...
        xa = mVertices[ indx - 1 ].at(localXCoordIndx);
        ya = mVertices[ indx - 1 ].at(localYCoordIndx);
        xb = mVertices[ indx ].at(localXCoordIndx);
        yb = mVertices[ indx ].at(localYCoordIndx);
...
PolylineNonlocalBarrier :: calculateMinimumDistanceFromBoundary(const FloatArray &coords)
{
    ...
    for ( int i = 1; i < size; i++ ) {
        ...
        for ( int j = 1; j <= coords.giveSize(); j++ ) {
            coordsA.at(j) =  mVertices[ i - 1 ].at(j);
            coordsB.at(j) = mVertices[ i ].at(j);
...
void
PolylineNonlocalBarrier :: giveInputRecord(DynamicInputRecord &input)
{ 
    input.setRecordKeywordField( "polylinebarrier", number );
    ...
    input.setField(points, _IFT_PolylineNonlocalBarrier_points);
    input.setField(localXCoordIndx, _IFT_PolylineNonlocalBarrier_xcoordindx);
    input.setField(localYCoordIndx, _IFT_PolylineNonlocalBarrier_ycoordindx);

}

void PolylineNonlocalBarrier ::UpdateVertices(FloatArray &Points)
{
...
PolylineNonlocalBarrier :: initializeFrom(InputRecord *ir)
{
    ...
    FloatArray points;
    IR_GIVE_FIELD(ir, points, _IFT_PolylineNonlocalBarrier_points);
