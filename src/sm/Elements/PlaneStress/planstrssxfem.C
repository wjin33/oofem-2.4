...
void PlaneStress2dXfem :: addGaussPoints()
{
    ...
    if ( integrationRulesArray.size() == 1 ) {
        ...
        integrationRulesArray[ 1 ].reset( new GaussIntegrationRule(1, this, 1, 3) );
        this->giveCrossSection()->setupIntegrationPoints(* integrationRulesArray [ 1 ], this->numberOfGaussPoints, this);
    }
...
//                    }
        ...
        std::vector<std::vector<FloatArray>> IS;

        XfemStructuralElementInterface::giveSubtriangulationCompositeExportData(vtkPieces,IS, primaryVarsToExport, internalVarsToExport, cellVarsToExport, tStep);
...
void
PlaneStress2dXfem :: giveXFEMExportData(std::vector< VTKPiece > &vtkPieces, std::vector<std::vector<FloatArray>> &IS, IntArray &primaryVarsToExport, IntArray &internalVarsToExport, IntArray cellVarsToExport, TimeStep *tStep)
{
    XfemStructuralElementInterface::giveSubtriangulationCompositeExportData(vtkPieces,IS, primaryVarsToExport, internalVarsToExport, cellVarsToExport, tStep);
}
