...
void XfemStructuralElementInterface :: giveSubtriangulationCompositeExportData(std :: vector< VTKPiece > &vtkPieces, std::vector<std::vector<FloatArray>> &IS, IntArray &primaryVarsToExport, IntArray &internalVarsToExport, IntArray cellVarsToExport, TimeStep *tStep)
{
    vtkPieces.resize(1);
...
//                        }
    ...
    vtkPieces [ 0 ].setNumberOfInternalVarsToExport(internalVarsToExport.giveSize(), numTotalNodes);
    ...
    for ( int fieldNum = 1; fieldNum <= internalVarsToExport.giveSize(); fieldNum++ ) {
        ...
        for ( auto &tri: mSubTri ) {
            for ( int i = 1; i <= 3; i++ ) {
                    ...
                    for (int elNodeInd = 1; elNodeInd <= nDofMan; elNodeInd++){
                        FloatArray Temp=0;
                        Temp.beScaled(N.at(elNodeInd), IS[fieldNum-1][elNodeInd-1]);
                        ISTemp += Temp ;
                    }

                    vtkPieces [ 0 ].setInternalVarInNode(fieldNum, nodesPassed, ISTemp);
                    nodesPassed++;
