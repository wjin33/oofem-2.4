...
#include "../sm/EngineeringModels/staticstructuralarc.h"
...
namespace oofem {
REGISTER_EngngModel(staticstructuralarc);

staticstructuralarc :: staticstructuralarc(int i, EngngModel *_master) : LinearStatic(i, _master),
...
staticstructuralarc :: ~staticstructuralarc()
{
    delete nMethod;
}


NumericalMethod *staticstructuralarc :: giveNumericalMethod(MetaStep *mStep)
{
    if ( !nMethod ) {
    ...
    int _val = 0;
    IR_GIVE_OPTIONAL_FIELD( ( mStep->giveAttributesRecord() ), _val, _IFT_staticstructuralarc_controlmode );
    ...
    staticstructuralarc_controlType mode = ( staticstructuralarc_controlType ) _val;
...
void
staticstructuralarc :: updateAttributes(MetaStep *mStep)
{
    IRResultType result;                  // Required by IR_GIVE_FIELD macro
     ...
     * double factor;
     *
     * printf ("staticstructuralarc: fixed load level");
    ...
    IR_GIVE_OPTIONAL_FIELD(ir, _val, _IFT_staticstructuralarc_controlmode);
    ...
    this->controlMode = ( staticstructuralarc_controlType ) _val;
    ...
    deltaT = 1.0;
    IR_GIVE_OPTIONAL_FIELD(ir, deltaT, _IFT_staticstructuralarc_deltat);
    if ( deltaT < 0. ) {
        OOFEM_ERROR("deltaT < 0");
    }

    _val = nls_tangentStiffness;
    IR_GIVE_OPTIONAL_FIELD(ir, _val, _IFT_staticstructuralarc_stiffmode);
    this->stiffMode = ( staticstructuralarc_stiffnessMode ) _val;

    _val = SparseNonLinearSystemNM :: rlm_total;
    IR_GIVE_OPTIONAL_FIELD(ir, _val, _IFT_staticstructuralarc_refloadmode);
    this->refLoadInputMode = ( SparseNonLinearSystemNM :: referenceLoadInputModeType ) _val;

    mstepCumulateLoadLevelFlag = ir->hasField(_IFT_staticstructuralarc_keepll);

    // called just to mark field as recognized, used later
    ir->hasField(_IFT_staticstructuralarc_donotfixload);
}


IRResultType
staticstructuralarc :: initializeFrom(InputRecord *ir)
{
    IRResultType result;                // Required by IR_GIVE_FIELD macro
    ...
    IR_GIVE_OPTIONAL_FIELD(ir, nonlocalStiffnessFlag, _IFT_staticstructuralarc_nonlocstiff);

    updateElasticStiffnessFlag = false;
    if ( ir->hasField(_IFT_staticstructuralarc_updateElasticStiffnessFlag) ) {
      updateElasticStiffnessFlag = true;
    }

    mRecomputeStepAfterPropagation = ir->hasField(_IFT_staticstructuralarc_recomputeaftercrackpropagation);
    ...
    if ( isParallel() ) {
        ...
        if ( ir->hasField(_IFT_staticstructuralarc_nonlocalext) ) {
            nonlocalExt = 1;
...
double staticstructuralarc :: giveUnknownComponent(ValueModeType mode, TimeStep *tStep, Domain *d, Dof *dof)
...
TimeStep *staticstructuralarc :: giveSolutionStepWhenIcApply(bool force)
{
  if ( master && (!force)) {
...
TimeStep *staticstructuralarc :: giveNextStep()
{
...
void staticstructuralarc :: solveYourself()
{
    if ( this->isParallel() ) {
...
void
staticstructuralarc :: solveYourselfAt(TimeStep *tStep)
{
    proceedStep(1, tStep);
}
...
void
staticstructuralarc :: terminate(TimeStep *tStep)
{
...
void
staticstructuralarc :: updateLoadVectors(TimeStep *tStep)
{
    ...
    if ( controlMode == nls_indirectControl ) {
        ...
        if ( isLastMetaStep ) {
            if ( !mstep->giveAttributesRecord()->hasField(_IFT_staticstructuralarc_donotfixload) ) {
    ...
    // if (isLastMetaStep) {
    if ( isLastMetaStep && !mstep->giveAttributesRecord()->hasField(_IFT_staticstructuralarc_donotfixload) ) {
#ifdef VERBOSE
...
void
staticstructuralarc :: proceedStep(int di, TimeStep *tStep)
{
    //
    ...
    if ( !stiffnessMatrix ) {
        ...
        if ( tStep->giveNumber() == 1 ) {
            ...
            totalDisplacement.resize(neq);
            this->field->initialize(VM_Total, tStep, this->totalDisplacement, EModelDefaultEquationNumbering() );
            this->field->update(VM_Total, tStep, this->totalDisplacement, EModelDefaultEquationNumbering() );
            incrementOfDisplacement.resize(neq);
        }
...
void
staticstructuralarc :: updateComponent(TimeStep *tStep, NumericalCmpn cmpn, Domain *d)
//
...
void
staticstructuralarc :: printOutputAt(FILE *File, TimeStep *tStep)
{
...
contextIOResultType
staticstructuralarc :: saveContext(DataStream *stream, ContextMode mode, void *obj)
//
...
contextIOResultType
staticstructuralarc :: restoreContext(DataStream *stream, ContextMode mode, void *obj)
//
...
//
{
    ...
    controlMode = ( staticstructuralarc_controlType ) _cm;
...
void
staticstructuralarc :: updateDomainLinks()
{
...
void
staticstructuralarc :: assemble(SparseMtrx &answer, TimeStep *tStep, const MatrixAssembler &ma,
...
#ifdef TIME_REPORT
    timer.stopTimer();
    OOFEM_LOG_DEBUG( "staticstructuralarc: User time consumed by assembly: %.2fs\n", timer.getUtime() );
#endif
}
...
#ifdef __OOFEG
void
staticstructuralarc :: showSparseMtrxStructure(int type, oofegGraphicContext &gc, TimeStep *tStep)
{
...
void
staticstructuralarc :: computeExternalLoadReactionContribution(FloatArray &reactions, TimeStep *tStep, int di)
{
...
void
staticstructuralarc :: assembleIncrementalReferenceLoadVectors(FloatArray &_incrementalLoadVector,
...
int
staticstructuralarc :: estimateMaxPackSize(IntArray &commMap, DataStream &buff, int packUnpackType)
{
...
#ifdef __PARALLEL_MODE
LoadBalancer *
staticstructuralarc :: giveLoadBalancer()
{
    if ( lb ) {
        return lb;
    }
...
LoadBalancerMonitor *
staticstructuralarc :: giveLoadBalancerMonitor()
{
    if ( lbm ) {
        return lbm;
    }
...
void
staticstructuralarc :: packMigratingData(TimeStep *tStep)
{
    Domain *domain = this->giveDomain(1);
...
void
staticstructuralarc :: unpackMigratingData(TimeStep *tStep)
{
    Domain *domain = this->giveDomain(1);
    int ndofman = domain->giveNumberOfDofManagers();
...
void staticstructuralarc :: setSolution(TimeStep *tStep, const FloatArray &vectorToStore)
{
    this->field->update(VM_Total, tStep, vectorToStore, EModelDefaultEquationNumbering());
}

int
staticstructuralarc :: forceEquationNumbering()
{
